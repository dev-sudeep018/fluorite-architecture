...
### `/paper`
- **`vivy_architecture.pdf`** — The main document. 27 pages. Everything found, everything that failed, the architecture proposal, cross-context values, independent validation against BDH, associative memory capacity, a correction to earlier momentum claims with a verified fix, identity under coercion (the central finding), and its resolution of the coordinated-pressure limitation. Read this first.
...
| `test_agency_gate_coordinated.js` | The same fix, applied to the earlier coordinated-pressure limitation. Full 8-seed result: 0.911 probe-hold, 0.938 genuine-recovery — beats both unprotected momentum and the EMA baseline that momentum originally lost to. Mechanism: gradient learning's natural inertia against short-duration evidence, inherited for free. |
...
**Cross-context values emerge — and the coordinated-pressure limit is resolved.** In a 4-context relational task with a cross-context consistency bonus, the momentum architecture developed a uniform 91–93% identity strength in every context simultaneously — the closest thing to a genuine value this program has produced. Coordinated pressure across all contexts at once originally defeated this advantage (EMA recovered faster: 0.813 vs momentum's 0.656). **Resolved**: the same agency-gating mechanism that fixes coercion also fixes this, for an unexpected reason — gradient learning has natural inertia against short-duration evidence, and agency-gating inherits that inertia for free. Agency-gated momentum reaches 0.911 probe-hold and 0.938 genuine-recovery, beating both unprotected momentum and the EMA baseline it originally lost to. Full 8-seed result in `test_agency_gate_coordinated.js`.
